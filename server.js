...
之後若超過門檻會自動提醒：
單檔報酬率變動 ${formatPercent(INTRADAY_STOCK_MOVE_PERCENT)}%
單檔損益變動 ${formatMoney(INTRADAY_STOCK_PROFIT_DELTA)} 元
總損益變動 ${formatMoney(INTRADAY_TOTAL_PROFIT_DELTA)} 元`
    };
  }
  ...
  for (const item of state.stocks.values()) {
    ...
    if (
      Math.abs(profitPercentDelta) >= INTRADAY_STOCK_MOVE_PERCENT ||
      Math.abs(profitDelta) >= INTRADAY_STOCK_PROFIT_DELTA
    ) {
      stockAnomalies.push({
        ...item,
        profitDelta,
  ...
  const totalTriggered =
    Math.abs(totalProfitDelta) >= INTRADAY_TOTAL_PROFIT_DELTA;
